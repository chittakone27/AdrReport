...
const Gender = ({ orgUnitId, startDate, endDate }) => {
  const { auth } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const params = {
          dimension: [`ou:${orgUnitId}`, "DmuazFb368B"], // orgUnit + gender
          stage: "d3iJyrWjNUy",
          startDate,
          endDate,
  ...
  "https://dhis2.asia/laotracker/api/29/analytics/events/aggregate/AQBx2QVBvRH.json",
  {
    ...
    params,
        ...
        const rawData = rows.map((r) => {
          const gender = r[0] === "M" ? "Male" : "Female";
  ...
  const pieConfig = {
    data: data.map((d) => ({ type: d.gender, value: d.percent })),
    angleField: "value",
    colorField: "type",
    radius: 0.8,
    height: 350,
  ...
  return (
    <Row gutter={16}>
      <Col span={14}>
        <Card title="Patient Gender Distribution" bordered style={{ height: "100%" }}>
          <Pie {...pieConfig} />
        </Card>
      </Col>
      <Col span={10}>
        <Card title="Gender Details" bordered style={{ height: "100%" }}>
          <Table
            size="small"
            bordered
            pagination={false}
